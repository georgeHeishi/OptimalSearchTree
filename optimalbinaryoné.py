...
def optimal_bst(p_table, q_table, n):
    e_table = [[0 for _ in range(n)] for _ in range(n + 1)]
    w_table = [[0 for _ in range(n)] for _ in range(n + 1)]
    root_table = [[0 for _ in range(n)] for _ in range(n + 1)]
    ...
    for l in range(1, n):
        for i in range(1, n - l + 1):
            ...
            w_table[i][j] = w_table[i][j - 1] + p_table[j] + q_table[j]
